...
const Navbar = ({ onMenuClick }) => {
    ...
    const fetchAdminData = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:3000/api/admin/${user.email}`
        );
  ...
  useEffect(() => {
      ...
      try {
        ...
        const filteredReports = adminData.department
          ? allReports.filter((report) =>
              adminData.department.some(
                (dept) =>
                  dept.toLowerCase().trim() ===
                  report.department.toLowerCase().trim()
              )
            )
          : allReports;
    ...
    if (adminData) fetchReports();
    const interval = setInterval(fetchReports, 60000);

    return () => clearInterval(interval);
  }, [adminData, unreadReports]);
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-[rgba(255,255,255,0.72)] backdrop-blur-xl border-b border-[#86868b]/10 z-50">
        ...
        <div className="flex justify-between h-12 items-center">
          <div className="flex items-center space-x-4">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="p-2 rounded-full text-[#86868b] lg:hidden hover:text-black hover:bg-[#f5f5f7] transition-all duration-300"
            >
              <Menu
                size={20}
            ...
            <Link to="/" className="flex items-center space-x-2 group">
              ...
              >
                <Siren className="w-6 h-6 text-black transform group-hover:scale-110 transition-all duration-300" />
                {unreadReports > 0 && (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{
              ...
              <motion.span
                className="hidden md:inline-block text-base font-semibold text-black"
          ...
          <div className="flex items-center space-x-4">
            <div className="relative" ref={bellRef}>
              <motion.button
                ...
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
                className="relative p-2 rounded-full hover:bg-[#f5f5f7] transition-all duration-300"
              >
                <motion.div
                  animate={
                    unreadReports > 0
              ...
              <AnimatePresence>
                {isBellOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 bg-white rounded-2xl shadow-lg border border-[#86868b]/10"
                    style={{
                      ...
                      <div className="p-4">
                        ...
                        {unreadReports > 0 ? (
                          ...
                          >
                            <Link
                              to="/reports"
                              className="block p-3 rounded-xl bg-gradient-to-r from-[#fff5f5] to-[#ffebeb] hover:from-[#ffeeee] hover:to-[#ffdada] transition-colors duration-300"
                            >
            ...
            <div className="relative" ref={profileRef}>
              <motion.button
                ...
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
                className="relative w-8 h-8 flex items-center justify-center rounded-full bg-[#1d1d1f] text-white text-sm font-medium transition-all duration-300"
              >
              ...
              <AnimatePresence>
                {isProfileOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    transition={{ type: "spring", stiffness: 300, damping: 30 }}
                    className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-lg border border-[#86868b]/10 overflow-hidden"
                  >
                    <motion.button
                      onClick={logout}
                      whileHover={{
